...
const Header = () => {
  const { user, backendStatus, mockLogout } = useAuth();
        ...
        {user ? (
          <div className="flex items-center space-x-3">
            <span className="text-sm text-slate-300">
              Hello, <span className="font-semibold text-violet-400">{user.username}</span>
            </span>
            <button
              onClick={mockLogout}
              className="text-xs bg-rose-600/25 hover:bg-rose-600/40 text-rose-300 px-3 py-1.5 rounded-md border border-rose-500/30 transition-all font-medium"
            >
              Logout
            </button>
          </div>
